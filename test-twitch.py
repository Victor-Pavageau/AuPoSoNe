...
def get_file_path(game_name, clip_index, isOriginal):
    ...
    return os.path.join(ROOT_PATH, game_name, subfolder, f"{game_name}-{clip_index}{VIDEO_FILE_EXTENSION}")
...
def crop_video(input_file_path, output_file_path):
    ...
    try:
        subprocess.run([
            ...
            "-lavfi", "[0:v]crop=ih*4/3:ih:(iw-ih*4/3)/2:0,scale=1080:-1[cropped];[cropped]scale=-1:1920,boxblur=luma_radius=min(h\\,w)/40:luma_power=3:chroma_radius=min(cw\\,ch)/40:chroma_power=1[bg];[bg][cropped]overlay=(W-w)/2:(H-h)/2,setsar=1,crop=w=1080:h=1920",
...
clips = get_clips(game_id, get_iso_formatted_datetime(yesterday), get_iso_formatted_datetime(now), CLIPS_COUNT)
